...
is made or until `txchangecoind` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `txchangecoin-wallet-cli` and
`txchangecoin-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--proxy` which tells `txchangecoind` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--proxy tor,127.0.0.1:9050,10`
> `--proxy i2p,127.0.0.1:9000`

which tells `txchangecoind` that ".onion" p2p addresses can be forwarded to a socks
...
`txchangecoind` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/i2p) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 38381 for mainnet) instead of the p2p port:

> HiddenServiceDir /var/lib/tor/data/txchangecoin
> HiddenServicePort 38381 127.0.0.1:38381
...
An ISP can passively monitor `txchangecoind` connections from a node and observe when
...
only using I2P for broadcasting Txchangecoin transactions, the total aggregate of
I2P data would also leak information.

#### Mitigation

There is no current mitigation for the user right now. This attack is fairly
sophisticated, and likely requires support from the internet host of a Txchangecoin
user.
...
### Intermittent Txchangecoin Syncing

If a user only runs `txchangecoind` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `txchangecoind` as often as possible to conceal when transactions are being sent.
