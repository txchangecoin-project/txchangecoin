...
*Setup instructions for a Gitian build of Txchangecoin.*

Gitian is the deterministic build process that is used to build the Txchangecoin CLI
...
```bash
cp txchangecoin/contrib/gitian/gitian-build.py .
```
...
```
git clone git@github.com:txchangecoin-project/gitian.sigs.git
...
gpg --output $VERSION-linux/$NAME/txchangecoin-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/txchangecoin-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/txchangecoin-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/txchangecoin-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/txchangecoin-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/txchangecoin-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[txchangecoin-project/gitian.sigs](https://github.com/txchangecoin-project/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/txchangecoin-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/txchangecoin-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/txchangecoin-osx-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/txchangecoinmooo-txchangecoin/bittxchangecoin -b txchangecoinmooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
