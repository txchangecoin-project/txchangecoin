...
Core tests take longer than any other txchangecoin tests, due to the high amount of computational work involved in validating core components.
...
To run only txchangecoin's core tests (after building):
...
To run only txchangecoin's crypto tests (after building):
...
```bash
txchangecoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `txchangecoin-wallet-rpc` with RPC port 38384. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only txchangecoin's hash tests (after building):
...
To run only txchangecoin's performance tests (after building):
...
To run only txchangecoin's unit tests (after building):
